...
- Um compilador C (gcc ou clang) com `CGO_ENABLED=1`, exigido pelo driver `github.com/mattn/go-sqlite3`. Sem CGO o projeto compila normalmente, mas o server falha na primeira requisição
  - macOS: `xcode-select --install`
  - Debian/Ubuntu: `sudo apt install build-essential`
- `sqlite3` (CLI), apenas para criar ou consultar o banco
...
O server **não cria a tabela**: ele espera um `cotacao.db` com a tabela `cotacao` no diretório de onde é executado. O banco não é versionado (`*db` está no `.gitignore`), então este passo é **obrigatório** depois de clonar o repositório:

```sh
./scripts/create_table.sh
```

O script cria `cmd/server/cotacao.db` com a tabela `cotacao` e pode ser executado de qualquer diretório. Rodar de novo não apaga dados (`CREATE TABLE IF NOT EXISTS`). Para criar o banco em outro lugar, passe o caminho: `./scripts/create_table.sh /caminho/cotacao.db`.
...
O servidor fica ouvindo em `http://localhost:8080` e loga cada requisição com o tempo de resposta.
...
| --- | --- | --- | --- |
| `dial tcp [::1]:8080: connect: connection refused` | — | O server não está rodando. | Suba o server (passo 2). Se ele encerrar sozinho, veja "Server encerra sem mensagem" abaixo. |
| `Get "http://localhost:8080/cotacao": EOF` | `no such table: cotacao` seguido de `http: panic serving ...` | O server foi iniciado fora de `cmd/server` (o SQLite cria um `cotacao.db` vazio no diretório atual) ou a tabela não existe. | Rode o server de dentro de `cmd/server` e, se precisar, recrie a tabela (passo 1). Apague o `cotacao.db` vazio que sobrou no diretório errado. |
| `Get "http://localhost:8080/cotacao": EOF` | `Binary was compiled with 'CGO_ENABLED=0', go-sqlite3 requires cgo to work. This is a stub` seguido de `http: panic serving ...` | O server foi compilado sem CGO. O build **não falha**; o erro só aparece na primeira requisição. | Instale um compilador C e rode com `CGO_ENABLED=1 go run server.go`. |
...
Nos dois casos de `EOF`, o `panic` no log é consequência do erro anterior (o handler segue usando um statement nulo); o server continua no ar e atende as próximas requisições.

### Server encerra sem mensagem

Se `go run server.go` voltar para o prompt imediatamente, sem log nenhum, a porta `8080` já está em uso: o erro de `ListenAndServe` não é logado e o processo sai com código `0`. Descubra quem ocupa a porta e encerre o processo:
